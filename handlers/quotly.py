...
async def quotly_func(client, message: Message):
    if not message.reply_to_message:
        return await message.reply_text("**Reply Pesan Untuk membuat quote**")
    if not message.reply_to_message.text:
        return await message.reply_text(
            "**Pesan yang Dibalas tidak memiliki teks apa pun! Tolong Reply Pesan Teks!**"
        )
    ...
    if len(message.command) < 2:
        ...
        await m.edit(
            "**Silakan ketik** `/help` **bila butuh Bantuan**"
        )
        return
    try:
